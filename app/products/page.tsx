...
export default function Page() {
  ...
  const fetchData = useCallback(async (currentBranch: string, range: { initDate: string; endDate: string }) => {
    ...
    setIsLoading(true);
    try {
      const url = `/products/${currentBranch}?start=${initDate}&end=${endDate}`;
      const res = await axios.get(url);
      setData(res.data.data);
    } catch (error) {
      toast.error(`Error al obtener datos de ventas: ${error}`)
      setData([]);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      <div className="mb-8">
        <h1 className="text-3xl font-bold tracking-tight text-foreground">Productos por Venta</h1>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_auto]">
        <Card>
          <CardHeader className="pb-4">
            ...
            <CardDescription>
              Selecciona el periodo para filtrar los productos.
            </CardDescription>
          </CardHeader>
        ...
        <Card className="lg:col-start-2 lg:row-start-1">
          ...
          <CardContent className="flex flex-col gap-4">
            <DownloadButton type='products' branch={branch} endDate={endDate} initDate={initDate} />
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="mt-6">
        <CardHeader className="pb-4">
          ...
          <CardDescription>
            {data.length} producto{data.length !== 1 ? "s" : ""} encontrado{data.length !== 1 ? "s" : ""} en el periodo seleccionado.
          </CardDescription>
        </CardHeader>
