...
export function DownloadButton({ initDate, endDate, clientIds }: DownloadSalesButtonProps) {
  ...
  const handleDownload = async () => {
    ...
    const downloadUrl = `/wholesale/debits/download?start=${initDate}&end=${endDate}&${clientIds.map(c => `ids=${c}`).join('&')}`;
    ...
    try {
      ...
      let filename = `Cobranza_${initDate}-${endDate}.xlsx`; // Nombre por defecto
