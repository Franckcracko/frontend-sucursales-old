...
import axios from '@/lib/axios-inventory'; // Asumo que es tu instancia configurada de Axios
import { Button } from '@/components/ui/button';
import { DownloadIcon, Loader2 } from 'lucide-react'; // Iconos de Lucide

export function DownloadButton({ date }: { date: string }) {
  const [isDownloading, setIsDownloading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const handleDownload = async () => {
    if (!date) {
      setError("Faltan parámetros de fecha.");
      return;
    }
    ...
    const downloadUrl = `/reports?date=${date}`;
    ...
    try {
      ...
      let filename = `Inventario_${date}.xlsx`; // Nombre por defecto
